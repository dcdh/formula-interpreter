...
public final class ListOfEvaluatedAtParameterResolver implements ParameterResolver {
    ...
    public record ListOfEvaluatedAt(List<EvaluatedAt> listOf) {
        public ListOfEvaluatedAt {
            Objects.requireNonNull(listOf);
            Validate.isTrue(!listOf.isEmpty());
        }

        public EvaluatedAt first() {
            return listOf.get(0);
        }

        public EvaluatedAt[] next() {
            return listOf.subList(1, listOf.size()).toArray(new EvaluatedAt[0]);
        }
    }
